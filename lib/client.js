...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  if (this.wsdl.options.forceSoap12Headers) {
    let portTypeName = Object.keys(this.wsdl.definitions.portTypes)[0];
    let contentType = `application/soap+xml; charset=utf-8; action="${this.wsdl.definitions.xmlns.__tns__}/${portTypeName}/${name}"`
  ...
  let soapHeaders = '';
  if (self.soapHeaders || self.security) {
    soapHeaders = ["<soap:Header>", (self.soapHeaders ? self.soapHeaders.join("\n") : ""), (self.security && !self.security.postProcess ? self.security.toXML() : ""), "</soap:Header>"].join('')
  }
  xml = `<?xml version="1.0" encoding="utf-8"?>
<soap:Envelope ${xmlnsSoap} xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" ${encoding} ${this.wsdl.xmlnsInEnvelope}> ${soapHeaders}
<soap:Body ${self.bodyAttributes ? self.bodyAttributes.join(' ') : ''} ${self.security && self.security.postProcess ? " Id='_0'" : ''}>
${message}
</soap:Body>
</soap:Envelope>`;
  ...
  self.lastRequestHeaders = req && req.headers;
};
